...
async def lifespan(app: FastAPI):
    print("[LearnFlow] Initializing database...")
    init_db()
    yield

app = FastAPI(title="LearnFlow AI Backend", lifespan=lifespan)
...
async def get_transcript(video_id: str, db: Session = Depends(get_session)):
    ...
    Checks SQL database first, then fetches from YouTube if not found.
    """
    # ── Step 0: Check Database First ──────────────────────────
    statement = select(Transcript).where(Transcript.video_id == video_id)
    existing = db.exec(statement).first()
    if existing:
        print(f"[LearnFlow] Found cached transcript for {video_id}")
        return {
    ...
    try:
        ...
        print(f"[LearnFlow] Saved transcript for {video_id} to database")
    except Exception as dbe:
        print(f"[LearnFlow] Database save failed: {dbe}")
        # We don't raise here, we still return the transcript fetched from YouTube
